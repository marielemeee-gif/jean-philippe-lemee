...
export default function ArchivesPage() {
  return <main id="top" className="document-page"><Header />
    ...
    <section className="archive-split shell"><div className="archive-column" id="parcours"><header><span>02</span><h2>Parcours</h2></header><div className="fact-list">{timeline.map(([year,text]) => <div key={year}><time>{year}</time><p>{text}</p></div>)}</div></div><div className="archive-column"><header><span>03</span><h2>Expositions choisies</h2></header><div className="fact-list">{exhibitionHighlights.map(([year,title]) => <div key={`${year}-${title}`}><time>{year}</time><p>{title}</p></div>)}</div></div></section>
